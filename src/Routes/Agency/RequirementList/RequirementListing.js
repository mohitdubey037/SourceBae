    ...
    <div>
      ...
      <div className={styles.searchBarContainer} >
        <div className={styles.searchBarStyle} >
          <SearchBar onChange={onSearch} bgColor={colors.WHITE} placeholder={'Type keyword here example “react js”'} />
        </div>
        <SizedBox width={'30px'} />
        <button className={`${buttonStyles.L_login} ${buttonStyles.nav_Lbutton} ${styles.searchBtn}`} >
          <text>Search</text>
        </button>
        <SizedBox width={'30px'} />
      ...
      <div className={styles.partition} >
        <div className={styles.listContainer} >
          {Array(6).fill('maq').map(() => <RequirementsCard des={{ des: data }} showButton={true} buttonTitle={'Select'} />)}
        </div>
