...
function RespondedDetails(props) {
  ...
  const agencyAction = (task) => {
    instance.patch(`/api/${Role}/hire-developers/update-matched-agency/${devId}`, { interested: task, agencyId })
      .then(response => {
        // setDisability(true)
                        ...
                        <div className="sharedDevDetails2">
                          <div>
                            {devData.agenciesMatched[0].interested === 0 ?
                              <>
                                <button className="acceptButtonShare" onClick={() => onOpenModal(devData._id)}>Accept or Reject</button>
